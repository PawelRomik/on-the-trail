...
import { useTextareaCaretFix } from "../../utils/misc/useTextareaCaretFix";

export default function NotesArea({ notes, setNotes }: { notes: string; setNotes: (val: string) => void }) {
	const { t } = useTranslation();

	const { onPointerDown } = useTextareaCaretFix();

	return (
		<textarea
			className='w-full h-full overflow-y-auto text-2xl p-4 book-scrollbar rounded-md focus:border-0 focus:outline-0 resize-none shadow-inner placeholder-zinc-700
