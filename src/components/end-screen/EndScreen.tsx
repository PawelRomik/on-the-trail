	...
	return (
		<div className='fixed inset-0 z-50 flex items-center justify-center bg-black/60'>
			...
			<div style={{ backgroundImage: `url(${stone})` }} className='shadow-red-900  relative w-full bg-size-[100%] max-w-3xl  text-white p-6 rounded-2xl shadow-md space-y-6'>
				<h1 className='text-3xl border-b-2 border-red-800 pb-2 bg-[rgba(0,0,0,0.4)] border-t-0 rounded-full w-auto font-bold text-center'>
					{state ? t("endScreen.win") : t("endScreen.lose")}
				</h1>

				<div className='flex w-full items-center justify-center gap-3'>
					<div className='flex-1 gap-2 relative flex flex-col items-center justify-center'>
						<p className='border-b-3 rounded-full px-3 border-red-800 text-3xl font-bold italic'>{t("endScreen.yourPick")}</p>
						<img src={characterImage} className=' bg-[rgba(0,0,0,0.9)] grayscale-100 border-4 border-zinc-950' />
						<div className='absolute bg-[rgba(0,0,0,0.7)] text-center py-3 italic w-full bottom-0 text-3xl'>{t("endScreen.rip")}</div>
					</div>
					<div className='flex-1 flex relative gap-2 flex-col items-center justify-center'>
						<p className='border-b-3 rounded-full px-3 border-red-800 text-3xl font-bold italic'>{t("endScreen.culprit")}</p>
						<img src={culpritImage} className={`bg-[rgba(0,0,0,0.9)] border-4 ${state && "grayscale-100"} border-zinc-950`} />
						{state && <div className='absolute bg-[rgba(0,0,0,0.7)] text-center py-3 italic w-full bottom-0 text-3xl'>{t("endScreen.rip")}</div>}
					</div>
				</div>
